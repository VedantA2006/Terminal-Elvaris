...
def call_ai_llm(provider: str, api_key: str, model: str, prompt: str, system_prompt: str = None, endpoint_url: str = None, temperature: float = 0.3) -> str:
    ...
    if provider == 'gemini':
        ...
        # Resilient candidate model fallback
        primary_model = model or default_models.get(provider, 'groq/openai/gpt-oss-120b')
        if provider == 'omniroute':
            if primary_model in ('auto/best-coding', 'auto/best-reasoning', 'auto'):
