...
from leaderboard import load_leaderboard, get_strategy_by_id, add_strategy_to_leaderboard, upgrade_leaderboard_to_mt5_data, get_date_slice_for_range
...
def _initialise():
    ...
    print("\n" + "=" * 65)
    ASSET = os.environ.get('ASSET', 'XAUUSD').upper()
    PORT = int(os.environ.get('PORT', 5000))
    print(f"  TRADINGVIEW PYTHON STRATEGY TERMINAL (MetaTrader 5 ECN {ASSET})")
    print("=" * 65)

    # 1. Load raw MT5 broker data
    print(f"\n[1/3] Loading MT5 Broker {ASSET} 5m data ...")
    raw_df = load_or_download()
    # Filter to Jan 2026 onwards for all charting and backtesting per user request
    raw_df = raw_df[raw_df.index >= '2026-01-01'].copy()
    _cache['raw_df'] = raw_df
    df_2026 = raw_df.copy()
    _cache['df_2026'] = df_2026
    print(f"      {len(raw_df):,} real candles ready (2026 YTD: {len(df_2026):,} candles)")

    # 2. Calculate baseline signals (Using Rank 1 Champion)
    print("[2/3] Computing baseline Rank #1 Champion signals ...")
    from leaderboard import load_leaderboard
    lb = load_leaderboard(raw_df, range_key='2026_ytd')
    rank1_code = DEFAULT_STRATEGY_CODE
    if lb and len(lb) > 0:
        rank1_code = lb[0].get('code', DEFAULT_STRATEGY_CODE)

    from strategy_executor import execute_strategy
    exec_res = execute_strategy(rank1_code, raw_df)
    
    _cache['df'] = raw_df
    _cache['trades'] = exec_res.get('trades', [])
    _cache['equity'] = exec_res.get('equity', [])
    _cache['stats'] = exec_res.get('stats', {'total_trades': 0, 'win_rate': 0.0, 'total_pnl': 0.0})
    ...
    base_stats = _cache['stats']
    print(f"      Total Trades: {base_stats.get('total_trades', 0)}, Win Rate: {base_stats.get('win_rate', 0)}%, Net PnL: ${base_stats.get('total_pnl', 0):,.2f}")
    print("\n" + "=" * 65)
    print(f"  Terminal is LIVE: http://127.0.0.1:{PORT}")
    print("=" * 65 + "\n")
...
def api_strategy_default():
    """Returns the default Python strategy code (now Rank 1 Champion)."""
    _initialise()
    from leaderboard import load_leaderboard
    lb = load_leaderboard(_cache.get('raw_df'), range_key='2026_ytd')
    code = DEFAULT_STRATEGY_CODE
    if lb and len(lb) > 0:
        code = lb[0].get('code', DEFAULT_STRATEGY_CODE)
...
def api_leaderboard():
    """Returns ranked strategies list with monthly R and Monte Carlo metrics for the selected backtest range."""
    _initialise()
    range_key = request.args.get('range', '2026_ytd')
    board = load_leaderboard(_cache.get('raw_df'), range_key=range_key)
    return jsonify({'success': True, 'leaderboard': board, 'range': range_key})


@app.route('/api/leaderboard/load', methods=['POST'])
def api_leaderboard_load():
    """Loads a strategy from leaderboard by ID or index and executes it on the active backtest range."""
    _initialise()
    data = request.get_json() or {}
    strat_id = str(data.get('id', '')).strip()
    idx = data.get('index')
    range_key = data.get('range', '2026_ytd')
    ...
    if not strat and (idx is not None or strat_id.isdigit()):
        try:
            lb = load_leaderboard(_cache.get('raw_df'), range_key=range_key)
            if idx is not None:
                target_idx = int(idx)
                if 0 <= target_idx < len(lb):
                    strat = lb[target_idx]
            else:
                # If strat_id is numeric, it is actually the rank (1-based)
                target_rank = int(strat_id)
                for item in lb:
                    if item.get('rank') == target_rank or item.get('id') == strat_id:
                        strat = item
                        break
        except Exception:
            pass
    ...
    target_df = get_date_slice_for_range(_cache.get('raw_df'), range_key=range_key)
    if target_df is None or len(target_df) < 100:
        target_df = _cache.get('df_2026', _cache.get('raw_df'))
...
# ---------------------------------------------------------------------------
# Calmar King 3 Live Broker Execution & OmniRoute Sentinel API
# ---------------------------------------------------------------------------

from calmar_king_3_live_engine import (
    CalmarKing3EnsembleEngine,
    verify_calmar_king_backtest_parity,
    run_omniroute_live_audit,
    edit_engine_with_omniroute,
    CALMAR_KING_CONFIG,
    SIGNALS_LOG_FILE,
    PARITY_AUDIT_FILE
)
...
class Calmar3DaemonManager:
    ...
    def engine(self):
        if self._engine is None:
            self._engine = CalmarKing3EnsembleEngine()
        return self._engine

    def start(self, poll_interval_sec: int = 10) -> bool:
        if self.is_running:
            return False
        self._stop_event.clear()
        self.is_running = True
        self.thread = threading.Thread(target=self._run_loop, args=(poll_interval_sec,), daemon=True)
        self.thread.start()
        return True
    ...
    def _run_loop(self, poll_interval: int):
        while not self._stop_event.is_set():
            try:
                res = self.engine.poll_live_feed()
                self.last_poll_result = res
                self.last_poll_time = datetime.now(timezone.utc).isoformat()
            except Exception as e:
                app.logger.warning(f"Calmar 3 live polling error: {e}")
            self._stop_event.wait(poll_interval)

calmar3_daemon = Calmar3DaemonManager()


@app.route('/api/calmar3/status', methods=['GET'])
def api_calmar3_status():
    try:
        broker_health = calmar3_daemon.engine.broker.check_health()
        live_tick = calmar3_daemon.engine.broker.fetch_live_tick()
        
        # Load latest parity audit if available
        parity_audit = None
        if PARITY_AUDIT_FILE.exists():
            try:
                with open(PARITY_AUDIT_FILE, 'r', encoding='utf-8') as f:
        ...
        return jsonify({
            'success': True,
            'daemon_running': calmar3_daemon.is_running,
            'last_poll_time': calmar3_daemon.last_poll_time,
            'last_poll_result': calmar3_daemon.last_poll_result,
...
@app.route('/api/calmar3/poll', methods=['POST'])
def api_calmar3_poll():
    try:
        res = calmar3_daemon.engine.poll_live_feed()
        calmar3_daemon.last_poll_result = res
        calmar3_daemon.last_poll_time = datetime.now(timezone.utc).isoformat()
        return jsonify({'success': True, 'poll_result': res})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500


@app.route('/api/calmar3/audit', methods=['POST'])
def api_calmar3_audit():
    try:
        # Runs full 100% parity verification and prompts OmniRoute LLM
        audit_res = run_omniroute_live_audit()
        return jsonify({'success': True, 'audit': audit_res})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500


@app.route('/api/calmar3/signals', methods=['GET'])
def api_calmar3_signals():
    try:
        signals = []
        if SIGNALS_LOG_FILE.exists():
            with open(SIGNALS_LOG_FILE, 'r', encoding='utf-8') as f:
...
@app.route('/api/calmar3/code', methods=['GET'])
def api_calmar3_code():
    try:
        engine_file = REPO_DIR / 'calmar_king_3_live_engine.py'
        code = engine_file.read_text(encoding='utf-8')
        return jsonify({'success': True, 'code': code, 'file': str(engine_file)})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500


@app.route('/api/calmar3/omniroute-edit', methods=['POST'])
def api_calmar3_omniroute_edit():
    try:
        data = request.get_json(silent=True) or {}
        instruction = data.get('instruction', '').strip()
        model = data.get('model', 'auto/best-fast')
        if not instruction:
            return jsonify({'success': False, 'message': 'Missing calibration instruction for OmniRoute.'}), 400
            
        res = edit_engine_with_omniroute(instruction, model=model)
        # Re-verify parity after edit
        audit_res = None
        if res.get('success'):
            try:
                audit_res = verify_calmar_king_backtest_parity()
            except Exception:
                pass
        return jsonify({'success': res.get('success', False), 'result': res, 'post_edit_audit': audit_res})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500


@app.route('/api/calmar3/toggle-daemon', methods=['POST'])
def api_calmar3_toggle_daemon():
    try:
        data = request.get_json(silent=True) or {}
        action = data.get('action') # 'start', 'stop', or 'toggle'
        if action == 'start':
            calmar3_daemon.start()
        elif action == 'stop':
            calmar3_daemon.stop()
        else:
            if calmar3_daemon.is_running:
                calmar3_daemon.stop()
            else:
                calmar3_daemon.start()
        return jsonify({
            'success': True,
            'daemon_running': calmar3_daemon.is_running,
            'message': f"Calmar King 3 live daemon is now {'RUNNING' if calmar3_daemon.is_running else 'STOPPED'}."
        })
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
# ---------------------------------------------------------------------------
# Rank #1 Champion Live Broker Execution & Equity Edge 50K Sentinel API
# ---------------------------------------------------------------------------

from rank1_rsi_divergence_live_engine import (
    Rank1RsiDivergenceEngine,
    verify_rank1_backtest_parity,
    edit_engine_with_omniroute as rank1_edit_engine_with_omniroute,
    RANK1_CONFIG,
    SIGNALS_LOG_FILE as RANK1_SIGNALS_LOG_FILE,
    PARITY_AUDIT_FILE as RANK1_PARITY_AUDIT_FILE
)


class Rank1DaemonManager:
    def __init__(self):
        self.is_running = False
        self.thread = None
        self._stop_event = threading.Event()
    ...
    def engine(self):
        if self._engine is None:
            self._engine = Rank1RsiDivergenceEngine()
        return self._engine
    ...
    def _run_loop(self, poll_interval: int):
        while not self._stop_event.is_set():
            try:
                ...
                app.logger.warning(f"Rank 1 live polling error: {e}")
            self._stop_event.wait(poll_interval)

rank1_daemon = Rank1DaemonManager()


@app.route('/api/rank1/status', methods=['GET'])
def api_rank1_status():
    try:
        broker_health = rank1_daemon.engine.broker.check_health()
        live_tick = rank1_daemon.engine.broker.fetch_live_tick()

        # Load latest parity audit if available
        parity_audit = None
        if RANK1_PARITY_AUDIT_FILE.exists():
            try:
                with open(RANK1_PARITY_AUDIT_FILE, 'r', encoding='utf-8') as f:
                    parity_audit = json.load(f)
            except Exception:
                pass
        ...
        # Load active signals
        signals = []
        if RANK1_SIGNALS_LOG_FILE.exists():
            try:
                with open(RANK1_SIGNALS_LOG_FILE, 'r', encoding='utf-8') as f:
                    signals = json.load(f)
            except Exception:
                signals = []
        ...
        return jsonify({
            'success': True,
            'strategy_name': RANK1_CONFIG['strategy']['name'],
            'rank': 1,
            'daemon_running': rank1_daemon.is_running,
            'last_poll_time': rank1_daemon.last_poll_time,
            'last_poll_result': rank1_daemon.last_poll_result,
            'broker_health': broker_health,
            'live_tick': live_tick,
            'parity_audit': parity_audit,
            'signals_count': len(signals),
            'recent_signals': signals[-10:] if signals else [],
            'account_model': RANK1_CONFIG['risk_management']['account_model'],
            'risk_management': {
                'initial_capital': RANK1_CONFIG['risk_management']['initial_capital'],
                'risk_pct_per_trade': RANK1_CONFIG['risk_management']['risk_pct_per_trade'],
                'max_risk_rule_pct': RANK1_CONFIG['risk_management']['max_risk_rule_pct'],
                'max_daily_loss_pct': RANK1_CONFIG['risk_management']['max_daily_loss_pct'],
                'max_trailing_loss_pct': RANK1_CONFIG['risk_management']['max_trailing_loss_pct'],
                'max_concurrent_trades': RANK1_CONFIG['risk_management']['max_concurrent_trades']
            }
        })
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
@app.route('/api/rank1/poll', methods=['POST'])
def api_rank1_poll():
    try:
        res = rank1_daemon.engine.poll_live_feed()
        rank1_daemon.last_poll_result = res
        rank1_daemon.last_poll_time = datetime.now(timezone.utc).isoformat()
        return jsonify({'success': True, 'poll_result': res})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500


@app.route('/api/rank1/parity', methods=['POST'])
def api_rank1_parity():
    try:
        audit_res = verify_rank1_backtest_parity()
        return jsonify({'success': True, 'audit': audit_res})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500


@app.route('/api/rank1/signals', methods=['GET'])
def api_rank1_signals():
    try:
        signals = []
        if RANK1_SIGNALS_LOG_FILE.exists():
            with open(RANK1_SIGNALS_LOG_FILE, 'r', encoding='utf-8') as f:
...
@app.route('/api/rank1/code', methods=['GET'])
def api_rank1_code():
    try:
        engine_file = REPO_DIR / 'rank1_rsi_divergence_live_engine.py'
        code = engine_file.read_text(encoding='utf-8')
        return jsonify({'success': True, 'code': code, 'file': str(engine_file)})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
@app.route('/api/rank1/toggle-daemon', methods=['POST'])
def api_rank1_toggle_daemon():
    try:
        data = request.get_json(silent=True) or {}
        action = data.get('action')  # 'start', 'stop', or 'toggle'
        if action == 'start':
        ...
        return jsonify({
            'success': True,
            'daemon_running': rank1_daemon.is_running,
            'message': f"Rank #1 live daemon is now {'RUNNING' if rank1_daemon.is_running else 'STOPPED'}."
        })
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
