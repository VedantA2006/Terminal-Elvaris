...
class SecurityASTVisitor(ast.NodeVisitor):
    ...
    def visit_While(self, node: ast.While):
        """Block while loops (potential for infinite loops)."""
        self.violations.append((
            node.lineno,
            "Prohibited use of 'while' loop. Strategies must use vectorized Pandas operations instead of slow manual loops."
        ))
        self.generic_visit(node)

    def visit_For(self, node: ast.For):
        """Block for loops (potential for infinite loops or incredibly slow iterrows)."""
        self.violations.append((
            node.lineno,
            "Prohibited use of 'for' loop. Strategies must use vectorized Pandas operations instead of slow manual loops."
        ))
        self.generic_visit(node)
