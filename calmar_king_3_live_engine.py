...
# ==============================================================================
CALMAR_KING_CONFIG = {
    # Calmar King 3 100% parity verified — parameters identical, no drift
    "version": "1.0.0",
    "last_calibrated": "2026-03-22T20:15:33Z",
    "calibrated_by": "OmniRoute API",
    "description": "Calmar King 3 Ensemble: Multi-Horizon LSS Hybrid + VWAP Pivots",  # Calmar King 3 100% parity verified
    "strategies": {  # Calmar King 3 100% parity verified — rank_2 / rank_8 / rank_9 unchanged
        "rank_2": {
            "enabled": True,
            "rank": 2,
        ...
        "rank_9": {
            "enabled": True,
            "rank": 9,
            "name": "Dual-Horizon Volatility Ratio Squeeze (ATR 5 / ATR 30 Expansion) (Round #21) (Optimized)",
            "swing_len": 7,
            "arm_period": 10,
            "atr_period": 14,
            "atr_multiplier": 1.8,
    ...
    "risk_management": {
        # Equity Edge Instant Funded 50K Account Specifications & Strict Guards
        "account_model": "Equity Edge Instant Funded 50K",
        "initial_capital": 50000.0,
        "max_risk_rule_pct": 1.0,           # Equity Edge Rule: Max risk 1% per trade ($500.00 max)
        "risk_pct_per_trade": 0.50,         # Conservative 0.50% ($250.00/trade) — guarantees 3 concurrent trades lose max 1.50%, far below 3% daily limit
        "max_daily_loss_pct": 3.0,          # Equity Edge Rule: Maximum Daily Loss 3% ($1,500.00)
        "max_daily_loss_circuit_breaker_usd": 1250.0, # Halts new entries at 2.5% to protect the 3% limit
        "max_trailing_loss_pct": 5.0,       # Equity Edge Rule: Maximum Trailing Loss 5% ($2,500.00)
        "max_trailing_loss_circuit_breaker_usd": 2250.0, # Halts new entries at 4.5% to protect 5% trailing line
        ...
        "max_concurrent_trades": 3          # Max 3 concurrent sub-strategies (Rank 2, 8, 9)
    },
    "broker_connection": {  # Calmar King 3 100% parity verified
    ...
    "omniroute_api": {  # Calmar King 3 100% parity verified
...
class CalmarKing3EnsembleEngine:
        ...
        if SIGNALS_LOG_FILE.exists():
            try:
                with open(SIGNALS_LOG_FILE, 'r', encoding='utf-8') as f:
                    ...
                    # Rebuild open positions for active signals
                    for s in self.active_signals:
                        if s.get('status') == 'ACTIVE_SIGNAL':
                            self.open_positions[s['rank']] = s
            except Exception:
                self.active_signals = []
                self.open_positions = {}
    ...
    def evaluate_signals_on_dataframe(self, df: pd.DataFrame, on_closed_candle: bool = True) -> Dict[str, Any]:
        ...
        # If on_closed_candle is True and we have at least 2 bars, evaluate the confirmed closed bar (iloc[-2])
        ...
        capital = float(self.config['risk_management'].get('initial_capital', 50000.0))
        ...
        max_concurrent = int(self.config['risk_management'].get('max_concurrent_trades', 3))
        ...
        for rank, strat_cfg, sig_df in evaluations:
            ...
            # Strict Backtest Parity: Only 1 concurrent position per strategy (matches backtest position is None rule)
            if rank in self.open_positions:
                active_pos = self.open_positions[rank]
                if active_pos.get('status') == 'ACTIVE_SIGNAL':
                    continue

            # Equity Edge Max Concurrent Trades Guard
            current_active_count = len([p for p in self.open_positions.values() if p.get('status') == 'ACTIVE_SIGNAL'])
            if current_active_count >= max_concurrent:
                logger.info(f"Max concurrent trades reached ({current_active_count}/{max_concurrent}). Skipping new entry for Rank #{rank}.")
                continue
            ...
            if bull and not bear:
                ...
                sig_obj = {
                    'id': f"CK3-R{rank}-{int(target_idx.timestamp())}",
                    'rank': rank,
    ...
    def poll_live_feed(self, on_closed_candle: bool = True) -> Dict[str, Any]:
        ...
        # 1. Update Open Positions with Real-Time Tick (Live Exit & SL/TP Tracking)
        closed_trades = []
        if tick and 'bid' in tick and 'ask' in tick:
            ...
            ranks_to_close = []
            for rank, pos in list(self.open_positions.items()):
                if pos.get('status') != 'ACTIVE_SIGNAL':
                    ranks_to_close.append(rank)
                    continue
                ...
                if pos['direction'] == 'BUY':
                    if bid <= pos['sl']:
                        ...
                        pos['realized_pnl_usd'] = round((pos['sl'] - pos['entry_price']) * pos.get('position_size_oz', 100.0), 2)
                        pos['realized_r'] = -1.0
                        logger.info(f"🛑 [TRADE CLOSED SL] Rank #{rank} BUY closed at {pos['sl']} (Loss: ${pos['realized_pnl_usd']})")
                        ranks_to_close.append(rank)
                        ...
                        pos['exit_time'] = tick_time
                        pos['realized_pnl_usd'] = round((pos['tp'] - pos['entry_price']) * pos.get('position_size_oz', 100.0), 2)
                        pos['realized_r'] = pos.get('rr_ratio', 3.0)
                        logger.info(f"🎯 [TRADE CLOSED TP] Rank #{rank} BUY closed at {pos['tp']} (Profit: +${pos['realized_pnl_usd']})")
                        ranks_to_close.append(rank)
                        closed_trades.append(pos)
                    ...
                    if ask >= pos['sl']:
                        ...
                        pos['exit_price'] = pos['sl']
                        pos['exit_time'] = tick_time
                        pos['realized_pnl_usd'] = round((pos['entry_price'] - pos['sl']) * pos.get('position_size_oz', 100.0), 2)
                        pos['realized_r'] = -1.0
                        logger.info(f"🛑 [TRADE CLOSED SL] Rank #{rank} SELL closed at {pos['sl']} (Loss: ${pos['realized_pnl_usd']})")
                        ranks_to_close.append(rank)
                        closed_trades.append(pos)
                    elif ask <= pos['tp']:
                        pos['status'] = 'CLOSED_TP'
                        pos['exit_price'] = pos['tp']
                        pos['exit_time'] = tick_time
                        pos['realized_pnl_usd'] = round((pos['entry_price'] - pos['tp']) * pos.get('position_size_oz', 100.0), 2)
                        pos['realized_r'] = pos.get('rr_ratio', 3.0)
                        logger.info(f"🎯 [TRADE CLOSED TP] Rank #{rank} SELL closed at {pos['tp']} (Profit: +${pos['realized_pnl_usd']})")
                        ranks_to_close.append(rank)
                        closed_trades.append(pos)

            for r in ranks_to_close:
                self.open_positions.pop(r, None)
        ...
        # 3. Log New Signals and Register in Open Positions
        if eval_res['signals']:
            for s in eval_res['signals']:
                if not any(x['id'] == s['id'] for x in self.active_signals):
                    self.active_signals.append(s)
                    self.open_positions[s['rank']] = s
                    logger.info(f"🚨 [NEW SIGNAL] Rank #{s['rank']} {s['direction']} @ {s['entry_price']} | Lots: {s.get('lots', 1.0)} | SL: {s['sl']} | TP: {s['tp']} (RR: {s['rr_ratio']})")
            self._save_signals_log()
...
def edit_engine_with_omniroute(instruction: str, model: Optional[str] = None) -> Dict[str, Any]:
    ...
    2. Uses focused block targeting for fast sub-5s OmniRoute execution
    3. Validates Python AST for syntax and safety
    4. Atomically updates the file
    5. Returns diff, validation score, and execution status
    """
    ...
    s_pos = current_code.find('CALMAR_KING_CONFIG = {')
    e_marker = current_code.find('# END OMNIROUTE EDITABLE CONFIGURATION BLOCK', s_pos)
    e_pos = current_code.rfind('# ===', s_pos, e_marker) if e_marker != -1 else -1

    if s_pos != -1 and e_pos != -1:
        config_text = current_code[s_pos:e_pos].strip()

        system_prompt = (
            "You are OmniRoute's Quantitative Optimization Agent. "
            "You are calibrating the configuration dictionary CALMAR_KING_CONFIG for a live trading engine. "
            "RULES:\n"
            "1. Output ONLY the updated Python code starting with CALMAR_KING_CONFIG = { and ending with } inside a single ```python ``` block.\n"
            "2. Update 'last_calibrated' with the current UTC timestamp and 'calibrated_by': 'OmniRoute API'.\n"
            "3. Strictly maintain valid Python dictionary syntax and valid types (floats, ints, booleans, strings).\n"
            "4. Output NO conversational text outside the markdown code fence."
        )

        user_prompt = f"""
USER CALIBRATION INSTRUCTION:
{instruction}
...
Return the complete, updated CALMAR_KING_CONFIG block inside ```python ```.
"""
        response_text = call_omniroute_llm(user_prompt, system_prompt=system_prompt, model=model or 'auto/best-fast')
        ...
        # AST validate snippet
        try:
            ast.parse(code_block)
        except SyntaxError as e:
            logger.error(f"OmniRoute generated invalid syntax: {e}")
            return {
                'success': False,
                'error': f"AST Syntax Error in OmniRoute config snippet: {e}",
                'backup_file': str(backup_file)
            }

        new_full_code = current_code[:s_pos] + code_block + "\n" + current_code[e_pos:]
    else:
        # Full file mode fallback
        system_prompt = (
            "You are OmniRoute's Senior Quantitative Engineering Agent. "
            "Output ONLY the complete Python file inside a single ```python ``` block."
        )
        user_prompt = f"INSTRUCTION:\n{instruction}\n\nCURRENT CODE:\n```python\n{current_code}\n```"
        response_text = call_omniroute_llm(user_prompt, system_prompt=system_prompt, model=model or 'auto/best-fast')
        new_full_code = response_text
        if "```python" in response_text:
            parts = response_text.split("```python")
            new_full_code = parts[1].split("```")[0].strip()

    # Full AST validation
    try:
        ast.parse(new_full_code)
    except SyntaxError as e:
        logger.error(f"Full AST Syntax Error: {e}")
        return {
            'success': False,
            'error': f"Full file AST Syntax Error: {e}",
            'backup_file': str(backup_file)
        }
    ...
    diff_lines = list(difflib.unified_diff(
        ...
        new_full_code.splitlines(keepends=True),
    ...
    # Atomically write
    engine_file.write_text(new_full_code, encoding='utf-8')
