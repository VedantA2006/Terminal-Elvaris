...
def run_optimization():
    ...
    # Load top 50 strategies
    all_strats = load_leaderboard(df, range_key='2026_ytd')
    top_50 = all_strats[:50]
    total_strats = len(top_50)
    print(f"Loaded top {total_strats} strategies for optimization.")

    for i, strat in enumerate(top_50):
        print(f"\n[{i+1}/{total_strats}] Optimizing '{strat['name']}'...")
