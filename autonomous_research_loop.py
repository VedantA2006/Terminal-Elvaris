...
class ResearchLoopManager:
    ...
    def get_state(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                "active_agent": {"XAUUSD": self.active_agent},
                "current_hypothesis": {"XAUUSD": self.current_hypothesis},
                "best_candidate": {"XAUUSD": self.best_candidate},
                "survivors_count": len(self.survivors_added),
                "recent_logs": {"XAUUSD": self.logs[-40:]},
                "engine": get_engine_telemetry(),
                "sentinel": self.get_sentinel_status(),
                "archetypes": {"XAUUSD": archs}
            }
    ...
    def _regenerate_archetypes(self, provider: str, api_key: str, model: str, endpoint: str):
        ...
        self._log("⚡ Alpha Sentinel", f"Initiating Autonomous Archetype Regeneration via OmniRoute ({model or 'best available'})...", "warning")
        try:
            ...
            base_prompt = f"""
We are running an autonomous trading AI that has plateaued. The current archetypes (like ICT, Price Action, Order Flow) have been completely saturated and exhausted.
We need you to invent 15 BRAND NEW, highly exotic market archetypes for {ASSET} 5-minute candles focusing EXCLUSIVELY on Deep Statistical Mathematics and Quantitative Math.
...
CRITICAL REQUIREMENT: You MUST focus on Deep Statistical Mathematics. 
Use concepts like: Z-Score Reversion, Advanced Linear Regression Curves, Kalman-like smoothing (using standard indicators), Standard Deviation Bands, Hurst Exponent logic, Variance/Volatility ratios, Multi-timeframe Statistical Arbitrage logic on the single asset, etc. Do NOT use retail chart patterns or ICT.
...
CRITICAL JSON RULES:
1. Do NOT use unescaped double quotes inside the "instructions" string. Use single quotes or escape them (e.g. \\").
2. Do NOT use raw newlines (\\n) inside the "instructions" string. If you want newlines, write them literally as '\\n'.
3. Do NOT embed markdown code blocks (like ```python) inside the JSON string values.
"""
            ...
            for attempt in range(1, max_attempts + 1):
                self._log("⚡ Alpha Sentinel", f"Regeneration Attempt {attempt}/{max_attempts}...", "info")
                ...
                if start >= 0 and end > start:
                    try:
                        new_archs = json.loads(json_str[start:end], strict=False)
                ...
                self._log("⚡ Alpha Sentinel", f"Attempt {attempt} failed: {error_msg}", "warning")
    ...
    def _run_tournament(self, provider: str, api_key: str, model: str, endpoint: str = None):
        ...
        try:
            ...
            while r <= self.max_rounds:
                ...
                batch_size = min(5, self.max_rounds - r + 1)
                added_to_top15_in_batch = False
                ...
                with ThreadPoolExecutor(max_workers=batch_size) as executor:
                    ...
                    for i in range(batch_size):
                        ...
                        futures.append(executor.submit(
                            self._execute_single_round, current_r, train_df, val_df, test_df, idea_agent, risk_agent, opt_agent
                        ))
                    ...
                    for future in concurrent.futures.as_completed(futures):
                        try:
                            ...
                            if res:
                                added_to_top15_in_batch = True
                        except concurrent.futures.TimeoutError:
                            self._log("System", f"⏰ A round timed out after 10 minutes (upstream LLM latency). Skipping...", "warning")
                        except Exception as round_err:
                            self._log("System", f"Round encountered error ({round_err}). Resuming...", "warning")
                
                if self.status == "paused" or self._should_pause:
                    return
                
                if not added_to_top15_in_batch:
                    self.rounds_since_top15_beat += batch_size
                else:
                    self.rounds_since_top15_beat = 0
                ...
                if self.rounds_since_top15_beat >= self.sentinel_threshold and not self.override_saturation:
                    ...
                    if self._regenerate_archetypes(provider, api_key, model, endpoint):
                        self.rounds_since_top15_beat = 0
    ...
    def _execute_single_round(self, r: int, train_df: pd.DataFrame, val_df: pd.DataFrame, test_df: pd.DataFrame,
                             idea_agent: IdeaGeneratorAgent, risk_agent: RiskOfficerAgent, opt_agent: OptimizerAgent) -> bool:
        """Executes a single end-to-end multi-agent discovery round. Returns True if strategy was added to leaderboard."""
        with self._lock:
        ...
        if r > 3 and winning_arch_indices and (r % 4 == 0):
            ...
            arch = load_archetypes()[archetype_idx]
            ...
            unsaturated_indices = [
                idx for idx, a in enumerate(load_archetypes())
                if a.get('id', a['name']) not in saturated_arch_ids
            ]
            if not unsaturated_indices:
                unsaturated_indices = list(range(len(load_archetypes())))

            # Select the least-attempted unsaturated archetype to systematically canvas all market regimes
            unsaturated_indices.sort(key=lambda idx: (self.archetype_tries.get(load_archetypes()[idx].get('id', load_archetypes()[idx]['name']), 0), idx))
            archetype_idx = unsaturated_indices[0]
            arch = load_archetypes()[archetype_idx]
            if saturated_arch_ids:
                sat_names = [a['name'] for a in load_archetypes() if a.get('id', a['name']) in saturated_arch_ids]
                self._log("🌐 Alpha Explorer", f"Exploration mode: Archetypes saturated in Leaderboard Top-15 ({', '.join(sat_names[:2])}). Cycling through novel domains across all {len(load_archetypes())} institutional archetypes: [{arch['name']}].", "info")
        ...
        def _evaluate_candidate(prop: Optional[Dict[str, Any]], target_arch: Dict[str, Any], target_idx: int, label: str):
            ...
            if total_trades > 0:
                ...
                if long_pct < 0.20 or short_pct < 0.20:
                    ...
                    if len(self.failure_memory) > 10:
                        self.failure_memory.pop(0)
                    self._log("dY>,? Risk Officer", f"Candidate [{target_arch['name']}]: Rejected due to directional bias (needs >= 20% both).", "warning")
                    return None
        ...
        min_val_trades = 0 # Disabled minimum validation trades
        if not val_exec.get("success"):
        ...
        if False: # Disabled forward-testing profitability constraints
        ...
        # Add to persistent leaderboard with 2026 backtest results (LOCKED for Thread Safety)
        with self._lock:
