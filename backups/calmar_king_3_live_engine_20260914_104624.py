...
def edit_engine_with_omniroute(instruction: str, model: Optional[str] = None) -> Dict[str, Any]:
    ...
    2. Uses focused block targeting (or full file) for fast sub-5s OmniRoute execution
    3. Validates Python AST for syntax and safety
    4. Atomically updates the file
    5. Re-runs parity check to verify zero regression
    6. Returns diff, validation score, and execution status
    """
    ...
    # Detect if instruction targets parameters, configuration, thresholds, or risk
    start_tag = "# === OMNIROUTE EDITABLE CONFIGURATION BLOCK"
    end_tag = "# === END OMNIROUTE EDITABLE CONFIGURATION BLOCK"

    if start_tag in current_code and end_tag in current_code:
        # Targeted Configuration Block Calibration (Lightning-fast, highly resilient)
        pre_block = current_code.split(start_tag)[0]
        rest = current_code.split(start_tag)[1]
        config_block = rest.split(end_tag)[0]
        post_block = rest.split(end_tag)[1]
        ...
        system_prompt = (
            ...
            "RULES:\n"
            "1. Output ONLY the updated Python code starting with CALMAR_KING_CONFIG = { and ending with } inside ```python ```\n"
            ...
            "4. No conversational filler or explanation outside the markdown code fence."
        )
...
"""
        ...
        new_full_code = f"{pre_block}{start_tag}\n{code_block}\n# ==============================================================================\n{end_tag}{post_block}"
    else:
        # Full file mode
        system_prompt = (
